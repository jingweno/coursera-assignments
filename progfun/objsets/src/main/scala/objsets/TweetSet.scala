...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = ascendingByRetweetAccu(new EmptyTrending)

  def ascendingByRetweetAccu(accu: Trending): Trending = {
    if (isEmpty) accu
    else remove(findMin).ascendingByRetweetAccu(accu + findMin)
  }
...
object GoogleVsApple {
  ...
  def tweetsContainKeyword(keyword: List[String]) = {
    allTweets.filter(tweet => {
      keyword.exists((text: String) => tweet.text.contains(text))
    })
  }

  val googleTweets: TweetSet = tweetsContainKeyword(google)

  val appleTweets: TweetSet = tweetsContainKeyword(apple)
